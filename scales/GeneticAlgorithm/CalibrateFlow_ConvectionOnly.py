...
# dummy peak
peak_exp = {}
peak_exp['time'] = np.arange(0, 100., dtype=float)
peak_exp['values'] = np.ones_like(peak_exp['time'])
peak_exp['values'][:int(peak_exp['time'].size/2)] = 0

# flow_rate = 0.001  # in m^3/s
delta_P = 500      # Pa
c_peak = 1.        # concentration in mol/m^3
D_bin = 1e-6*0     # binary diffusion coefficient in m^2/s
...
# dp = network['throat.diameter']
# network['pore.diameter'] = dp * F_range[1]
# network['pore.volume'] = math.pi * (network['pore.diameter'])**2 * spacing * 0.25

t_full = 100.
dt = t_full/float(peak_exp['time'].size)
peak_exp['time'] = np.arange(0., t_full, dt)
